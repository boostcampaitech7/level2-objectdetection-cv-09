...
import random
import Faster_rcnn.Train
import MMdetection.Train
import Detectron2.Train
...
def main(config, config_path):
    ...
    if config['model_type'] == 'torchvision':
        Faster_rcnn.Train.main(config)
    if config['model_type'] == 'mmdetection':
        MMdetection.Train.main(config)
    if config['model_type'] == 'detectron2':
        Detectron2.Train.main(config)
